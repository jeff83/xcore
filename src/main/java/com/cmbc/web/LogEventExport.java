...
import com.cmbc.entity.LogEvent;
import com.cmbc.entity.QLogEvent;
	...
	public void logEventExport(HttpServletResponse response, @RequestParam(required = false) final String level)
		...
		try (OutputStream out = response.getOutputStream(); PrintWriter pw = new PrintWriter(out)) {
			...
			if (StringUtils.isNotBlank(level)) {
				events = new JPAQuery(entityManager).from(QLogEvent.logEvent).where(QLogEvent.logEvent.level.eq(level))
						.list(QLogEvent.logEvent);
			} else {
				events = new JPAQuery(entityManager).from(QLogEvent.logEvent).list(QLogEvent.logEvent);
			}
