Ext.define('xcore.controller.Viewport', {
	extend: 'Deft.mvc.ViewController',
	requires: [ 'xcore.view.UserSettings' ],
	...
	onSettingsButtonClick: function() {
		if (xcore.user) {
			var userSettingsWindow = Ext.create('xcore.view.UserSettings');
			userSettingsWindow.down('form').loadRecord(Ext.create('xcore.model.User', xcore.user));
	...
	onUserSettingsSaveButtonClick: function(button) {
		...
		userService.updateSettings(form.getForm().getFieldValues(), function(result) {
			if (result.success) {
				win.close();
				xcore.ux.window.Notification.info(i18n.successful, i18n.settings_saved);
			}
		});
